...
// [ Parameters ]
//		DiscordID sql.NullString = NULLABLE
//		UserID int64 = REQUIRED
func (r *DiscordBot) Fill(DiscordID sql.NullString, UserID int64) {
	r.DiscordID = DiscordID
	r.UserID = UserID
}

// GetDiscordID retrieves struct field value,
...
func (r *DiscordBot) GetDiscordID() string {
	return util.FromNullString(r.DiscordID)
}

// GetUserID retrieves struct field value,
// if any field data type is int representing enum, then the corresponding enum is returned,
// if any field data type is sql null type or pointer, then the null or pointer is converted to base type
func (r *DiscordBot) GetUserID() int64 {
	return r.UserID
}

// SetDiscordID sets value into struct field value and checks for data input initial validation rules,
...
func (r *DiscordBot) SetDiscordID(v string) error {
	r.DiscordID = util.ToNullString(v, true)
...
func (r *DiscordBot) SetUserID(v int64) error {
	r.UserID = v
...
func (r *DiscordBot) GetByID(ID int64) (notFound bool, err error) {
	// clean up
	...
	q.Set("SELECT * FROM DiscordBot WHERE ID=? LIMIT 1;")
	q.Ordinal(ID)

	// execute query
	var dbCurrent *data.MySql

	if !_DiscordBotPrefersDBReader {
		dbCurrent = db
	} else {
...
// an error of nil indicates success
// note: this function will have struct tag ggAttr:"9 = GetBy" for columns acting as its source
func (r *DiscordBot) GetByUserID(UserID int64) (notFound bool, err error) {
	// clean up
	r._originalValue = nil
	...
	q.Set("SELECT * FROM DiscordBot WHERE UserID=? LIMIT 1;")
	q.Ordinal(UserID)
...
func (r *DiscordBot) GetScalar(scalarColumnName string, filter string, sort string, args ...interface{}) (result string, notFound bool, err error) {
	// validate
	...
	if !isFromOverride {
		q.Set("SELECT " + scalarColumnName + " FROM DiscordBot " + filter + sort + " LIMIT 1;")
	} else {
...
func (r *DiscordBot) IsDataChanged() bool {
	...
	old := r._originalValue.(DiscordBot)

	// check if old value vs current value was changed
	if r.DiscordID != old.DiscordID {
		return true
	}
...
func (r *DiscordBot) Validate() error {
...
func (r *DiscordBot) Set() error {
	...
	if r.ID <= 0 {
		...
		q.Set("INSERT INTO DiscordBot ")
		q.Set("(DiscordID, UserID) ")
		q.Set("VALUES ")
		q.Set("(:DiscordID, :UserID);")

		q.Named("DiscordID", r.DiscordID)
		...
		q.Set("UPDATE DiscordBot ")
		q.Set("SET DiscordID=:DiscordID, UserID=:UserID ")
		q.Set("WHERE ID=:ID;")
	...
	if tx, e = db.Begin(); e != nil {
		return errors.New("DiscordBot Persist Data Failed at BeginTran: " + e.Error())
	}
	...
	if result.Err != nil {
		if err1 := tx.Rollback(); err1 != nil {
			return errors.New("DiscordBot Persist Data Failed at RollBack 1: " + err1.Error() + " (Orig Error: " + result.Err.Error() + ")")
		} else {
			return errors.New("DiscordBot Persist Data Failed: " + result.Err.Error())
		}
	}
	...
	if !isNewRow {
		...
		if result.RowsAffected <= 0 {
			// update failure
			if err1 := tx.Rollback(); err1 != nil {
				return errors.New("DiscordBot Persist Data Failed at RollBack 2: " + err1.Error() + " (Orig Error: No Rows Affected)")
			} else {
				return errors.New("DiscordBot Persist Updated Data Failed: No Rows Affected")
			}
		}
	} else {
		// was insert
		...
		if result.NewlyInsertedID > 0 {
			...
			if err1 := tx.Rollback(); err1 != nil {
				return errors.New("DiscordBot Persist Data Failed at RollBack 3: " + err1.Error() + " (Orig Error: No New Primary ID Generated)")
			} else {
				return errors.New("DiscordBot Persist Inserted Data Failed: No New Primary ID Generated")
			}
		}
	}
	...
	if err1 := tx.Commit(); err1 != nil {
		return errors.New("DiscordBot Persist Data Failed at Commit: " + err1.Error())
	}
...
func (r *DiscordBot) physicalDelete() error {
	// compose query
	q := new(data.QueryBuilder)

	q.Set("DELETE FROM DiscordBot WHERE ID=?;")
	q.Ordinal(r.ID)
	...
	if result.Err != nil {
		return errors.New("DiscordBot Physical Delete Failed: " + result.Err.Error())
	}

	if result.RowsAffected <= 0 {
		return errors.New("DiscordBot Physical Delete Failed: No Rows Affected")
	}
...
func (r *DiscordBot) Delete() error {
	// validate
	if r.ID <= 0 {
		return errors.New("DiscordBot Delete Data Failed: Row Primary ID is Required")
	}

	if r._originalValue == nil {
		return errors.New("DiscordBot Delete Data Failed: Row Must Be Loaded First")
	}
...
// ---------------------------------------------------------------------------------------------------------------------
// DiscordBotList Actions
// ---------------------------------------------------------------------------------------------------------------------

// DiscordBotList collection level dal model
type DiscordBotList struct {
	List  *[]DiscordBot
	Count int
}

// indicates if DiscordBot list will prefer dbReader over dbWriter where available
var _DiscordBotListPrefersDBReader bool

// UseDBReaderPreferred sets indicator that this struct prefers to use db reader where applicable and if available
func (l DiscordBotList) UseDBReaderPreferred() {
	// set db reader usage preference to true, so if db reader is available, we will use it if code is applicable for it
	_DiscordBotListPrefersDBReader = true
}

// UseDBWriterPreferred sets indicator that this struct prefers to use db writer (this is the default preference)
func (l DiscordBotList) UseDBWriterPreferred() {
	// set db writer usage preference to true, so this struct list will use the db writer for operations
	_DiscordBotListPrefersDBReader = false
}

// Element gets one of the slice elements in list, identified by ordinal index position
func (l *DiscordBotList) Element(position int) (*DiscordBot, error) {
	// validate
	if l == nil {
...
func (l *DiscordBotList) GetAll(limitCount int, offsetCount ...int) error {
...
func (l *DiscordBotList) GetByID(IDToLoad ...int64) error {
	// validate
	if l == nil {
...
// [ Notes ]
// 		this function is based on the database column dbAttr:"GetBy" for suffix naming
func (l *DiscordBotList) GetByUserID(UserIDToLoad ...int64) error {
	// validate
	if l == nil {
		return errors.New("List Object Nil")
	}
...
func (l *DiscordBotList) GetCustom(filter string, sort string, offsetCount int, limitCount int, args ...interface{}) error {
	// validate
	if l == nil {
		return errors.New("List Object Nil")
	}
...
//		args = optional ordinal parameter arguments to pass into the filter query portion (must appear in ordinal position of params defined within filter)
func (l *DiscordBotList) getInternal(filter string, sort string, offsetCount int, limitCount int, args ...interface{}) error {
	// validate
	if l == nil {
		return errors.New("List Object Nil")
	}
	...
	if !isFromOverride {
		q.Set("SELECT * FROM DiscordBot " + filter + sort + limit + ";")
	} else {
	...
	if !_DiscordBotListPrefersDBReader {
		dbCurrent = db
	} else {
		dbCurrent = getReaderDB()
	}
...
func (l *DiscordBotList) IsDataChanged() (changedList *[]DiscordBot) {
	// validate
	if l == nil {
...
func (l *DiscordBotList) Validate() (invalid *DiscordBot, invalidInfo string, err error) {
	// validate
	if l == nil {
...
// a new database row will be inserted if such was not found,
// otherwise existing database row will be updated
func (l *DiscordBotList) Set() (failed *DiscordBot, err error) {
	// validate
	if l == nil {
		// if not valid, then return error
		return nil, errors.New("List Object Nil")
...
func (l *DiscordBotList) Delete() (failed *DiscordBot, err error) {
	// validate
	if l == nil {
		// if not valid, then return error
		return nil, errors.New("List Object Nil")
...
// [ Notes ]
//		this function is based on the database column dbAttr:"GetBy" for suffix naming
func (l *DiscordBotList) FindByID(IDToFind ...int64) (foundList *[]DiscordBot, err error) {
	// validate
	if l == nil {
		// if not valid, then return error
		return nil, errors.New("List Object Nil")
...
// [ Notes ]
//		this function is based on the database column dbAttr:"GetBy" for suffix naming
func (l *DiscordBotList) FindByUserID(UserIDToFind ...int64) (foundList *[]DiscordBot, err error) {
	// validate
	if l == nil {
		// if not valid, then return error
		return nil, errors.New("List Object Nil")
