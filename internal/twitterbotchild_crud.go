...
// [ Parameters ]
//		TwitterBotID int64 = REQUIRED
//		ImageURL string = REQUIRED
//		TextContent string = REQUIRED
//		Datetime time.Time = REQUIRED
//		AuthorTwitterID string = REQUIRED
//		AuthorTwitterTag string = REQUIRED
func (r *TwitterBotChild) Fill(TwitterBotID int64, ImageURL string, TextContent string, Datetime time.Time, AuthorTwitterID string, AuthorTwitterTag string) {
	r.TwitterBotID = TwitterBotID
	r.ImageURL = ImageURL
	r.TextContent = TextContent
	r.Datetime = Datetime
	r.AuthorTwitterID = AuthorTwitterID
	r.AuthorTwitterTag = AuthorTwitterTag
}

// GetTwitterBotID retrieves struct field value,
...
func (r *TwitterBotChild) GetTwitterBotID() int64 {
	return r.TwitterBotID
}

// GetImageURL retrieves struct field value,
// if any field data type is int representing enum, then the corresponding enum is returned,
// if any field data type is sql null type or pointer, then the null or pointer is converted to base type
func (r *TwitterBotChild) GetImageURL() string {
	return r.ImageURL
}
...
// if any field data type is int representing enum, then the corresponding enum is returned,
// if any field data type is sql null type or pointer, then the null or pointer is converted to base type
func (r *TwitterBotChild) GetTextContent() string {
...
// if any field data type is int representing enum, then the corresponding enum is returned,
// if any field data type is sql null type or pointer, then the null or pointer is converted to base type
func (r *TwitterBotChild) GetDatetime() time.Time {
	return r.Datetime
}

// GetAuthorTwitterID retrieves struct field value,
// if any field data type is int representing enum, then the corresponding enum is returned,
// if any field data type is sql null type or pointer, then the null or pointer is converted to base type
func (r *TwitterBotChild) GetAuthorTwitterID() string {
	return r.AuthorTwitterID
}

// GetAuthorTwitterTag retrieves struct field value,
// if any field data type is int representing enum, then the corresponding enum is returned,
// if any field data type is sql null type or pointer, then the null or pointer is converted to base type
func (r *TwitterBotChild) GetAuthorTwitterTag() string {
	return r.AuthorTwitterTag
}

// SetTwitterBotID sets value into struct field value and checks for data input initial validation rules,
...
func (r *TwitterBotChild) SetTwitterBotID(v int64) error {
	r.TwitterBotID = v
...
func (r *TwitterBotChild) SetImageURL(v string) error {
	if util.LenTrim(v) == 0 {
...
//		[ Parameters ]
//		v string = Struct internal data type: string; REQUIRED
func (r *TwitterBotChild) SetTextContent(v string) error {
	if util.LenTrim(v) == 0 {
...
func (r *TwitterBotChild) SetDatetime(v time.Time) error {
	if v.IsZero() {
...
// SetAuthorTwitterID sets value into struct field value and checks for data input initial validation rules,
// if any field data type is int representing enum, then the corresponding enum is used as input parameter,
// if any field data type is sql null type or pointer, then the base type is used as parameter and converted into null or pointer
//		[ Parameters ]
//		v string = Struct internal data type: string; REQUIRED
func (r *TwitterBotChild) SetAuthorTwitterID(v string) error {
	if util.LenTrim(v) == 0 {
		return errors.New("SetAuthorTwitterID Failed, A Text Length Greater Than Zero is Required")
	}
...
// SetAuthorTwitterTag sets value into struct field value and checks for data input initial validation rules,
// if any field data type is int representing enum, then the corresponding enum is used as input parameter,
// if any field data type is sql null type or pointer, then the base type is used as parameter and converted into null or pointer
//		[ Parameters ]
//		v string = Struct internal data type: string; REQUIRED
func (r *TwitterBotChild) SetAuthorTwitterTag(v string) error {
	if util.LenTrim(v) == 0 {
		return errors.New("SetAuthorTwitterTag Failed, A Text Length Greater Than Zero is Required")
	}

	r.AuthorTwitterTag = v
...
func (r *TwitterBotChild) GetByID(ID int64) (notFound bool, err error) {
	// clean up
	...
	q.Set("SELECT * FROM TwitterBotChild WHERE ID=? LIMIT 1;")
	q.Ordinal(ID)

	// execute query
	var dbCurrent *data.MySql

	if !_TwitterBotChildPrefersDBReader {
		dbCurrent = db
	} else {
...
// GetByTwitterBotID retrieves a row from database using the TwitterBotID field value,
// and marshals found result fields into struct,
// an error of nil indicates success
// note: this function will have struct tag ggAttr:"9 = GetBy" for columns acting as its source
func (r *TwitterBotChild) GetByTwitterBotID(TwitterBotID int64) (notFound bool, err error) {
	// clean up
	r._originalValue = nil
	...
	q.Set("SELECT * FROM TwitterBotChild WHERE TwitterBotID=? LIMIT 1;")
	q.Ordinal(TwitterBotID)
...
func (r *TwitterBotChild) GetScalar(scalarColumnName string, filter string, sort string, args ...interface{}) (result string, notFound bool, err error) {
	// validate
	...
	if !isFromOverride {
		q.Set("SELECT " + scalarColumnName + " FROM TwitterBotChild " + filter + sort + " LIMIT 1;")
	} else {
...
func (r *TwitterBotChild) IsDataChanged() bool {
	...
	old := r._originalValue.(TwitterBotChild)

	// check if old value vs current value was changed
	if r.TwitterBotID != old.TwitterBotID {
		return true
	}
	...
	if r.AuthorTwitterID != old.AuthorTwitterID {
		return true
	}

	if r.AuthorTwitterTag != old.AuthorTwitterTag {
		return true
	}
...
func (r *TwitterBotChild) Validate() error {
	// check for required fields
	// based on ggAttr:"3 = - (required)" and ggAttr:"6 = Set-Upd, Set-InsUpd, Set-Ins"
	if r.TwitterBotID == 0 {
		return errors.New("TwitterBotID is Required")
	}
	...
	if util.LenTrim(r.AuthorTwitterID) == 0 {
		return errors.New("AuthorTwitterID is Required")
	}

	if util.LenTrim(r.AuthorTwitterTag) == 0 {
		return errors.New("AuthorTwitterTag is Required")
	}
...
func (r *TwitterBotChild) Set() error {
	...
	if r.ID <= 0 {
		...
		q.Set("INSERT INTO TwitterBotChild ")
		q.Set("(TwitterBotID, ImageURL, TextContent, Datetime, AuthorTwitterID, AuthorTwitterTag) ")
		q.Set("VALUES ")
		q.Set("(:TwitterBotID, :ImageURL, :TextContent, :Datetime, :AuthorTwitterID, :AuthorTwitterTag);")

		q.Named("TwitterBotID", r.TwitterBotID)
		q.Named("ImageURL", r.ImageURL)
		q.Named("TextContent", r.TextContent)
		q.Named("Datetime", r.Datetime)
		q.Named("AuthorTwitterID", r.AuthorTwitterID)
		q.Named("AuthorTwitterTag", r.AuthorTwitterTag)
	} else {
		// update
		isNewRow = false

		// compose update action query
		q.Set("UPDATE TwitterBotChild ")
		q.Set("SET TwitterBotID=:TwitterBotID, ImageURL=:ImageURL, TextContent=:TextContent, Datetime=:Datetime, AuthorTwitterID=:AuthorTwitterID, AuthorTwitterTag=:AuthorTwitterTag ")
		q.Set("WHERE ID=:ID;")
	...
	if tx, e = db.Begin(); e != nil {
		return errors.New("TwitterBotChild Persist Data Failed at BeginTran: " + e.Error())
	}
	...
	if result.Err != nil {
		if err1 := tx.Rollback(); err1 != nil {
			return errors.New("TwitterBotChild Persist Data Failed at RollBack 1: " + err1.Error() + " (Orig Error: " + result.Err.Error() + ")")
		} else {
			return errors.New("TwitterBotChild Persist Data Failed: " + result.Err.Error())
		}
	}
	...
	if !isNewRow {
		...
		if result.RowsAffected <= 0 {
			// update failure
			if err1 := tx.Rollback(); err1 != nil {
				return errors.New("TwitterBotChild Persist Data Failed at RollBack 2: " + err1.Error() + " (Orig Error: No Rows Affected)")
			} else {
				return errors.New("TwitterBotChild Persist Updated Data Failed: No Rows Affected")
			}
		}
	} else {
		// was insert
		...
		if result.NewlyInsertedID > 0 {
			...
			if err1 := tx.Rollback(); err1 != nil {
				return errors.New("TwitterBotChild Persist Data Failed at RollBack 3: " + err1.Error() + " (Orig Error: No New Primary ID Generated)")
			} else {
				return errors.New("TwitterBotChild Persist Inserted Data Failed: No New Primary ID Generated")
			}
		}
	}
	...
	if err1 := tx.Commit(); err1 != nil {
		return errors.New("TwitterBotChild Persist Data Failed at Commit: " + err1.Error())
	}
...
func (r *TwitterBotChild) physicalDelete() error {
	// compose query
	q := new(data.QueryBuilder)

	q.Set("DELETE FROM TwitterBotChild WHERE ID=?;")
	q.Ordinal(r.ID)
	...
	if result.Err != nil {
		return errors.New("TwitterBotChild Physical Delete Failed: " + result.Err.Error())
	}

	if result.RowsAffected <= 0 {
		return errors.New("TwitterBotChild Physical Delete Failed: No Rows Affected")
	}
...
func (r *TwitterBotChild) Delete() error {
	// validate
	if r.ID <= 0 {
		return errors.New("TwitterBotChild Delete Data Failed: Row Primary ID is Required")
	}

	if r._originalValue == nil {
		return errors.New("TwitterBotChild Delete Data Failed: Row Must Be Loaded First")
	}
...
// ---------------------------------------------------------------------------------------------------------------------
// TwitterBotChildList Actions
// ---------------------------------------------------------------------------------------------------------------------

// TwitterBotChildList collection level dal model
type TwitterBotChildList struct {
	List  *[]TwitterBotChild
	Count int
}

// indicates if TwitterBotChild list will prefer dbReader over dbWriter where available
var _TwitterBotChildListPrefersDBReader bool

// UseDBReaderPreferred sets indicator that this struct prefers to use db reader where applicable and if available
func (l TwitterBotChildList) UseDBReaderPreferred() {
	// set db reader usage preference to true, so if db reader is available, we will use it if code is applicable for it
	_TwitterBotChildListPrefersDBReader = true
}

// UseDBWriterPreferred sets indicator that this struct prefers to use db writer (this is the default preference)
func (l TwitterBotChildList) UseDBWriterPreferred() {
	// set db writer usage preference to true, so this struct list will use the db writer for operations
	_TwitterBotChildListPrefersDBReader = false
}

// Element gets one of the slice elements in list, identified by ordinal index position
func (l *TwitterBotChildList) Element(position int) (*TwitterBotChild, error) {
	// validate
	if l == nil {
...
func (l *TwitterBotChildList) GetAll(limitCount int, offsetCount ...int) error {
...
func (l *TwitterBotChildList) GetByID(IDToLoad ...int64) error {
	// validate
	if l == nil {
...
// GetByTwitterBotID will load struct slice for one or more matching values of same parameter from database
// [ Parameters ]
//		TwitterBotIDToLoad = variadic, one or more table TwitterBotID to load from database, for example, enter 2, 3, 6, 9, will load records with TwitterBotID 2, 3, 6, and 9 from database to struct list
// [ Notes ]
// 		this function is based on the database column dbAttr:"GetBy" for suffix naming
func (l *TwitterBotChildList) GetByTwitterBotID(TwitterBotIDToLoad ...int64) error {
	// validate
	if l == nil {
		return errors.New("List Object Nil")
	}

	if TwitterBotIDToLoad == nil {
		return errors.New("Get Requires One or More Filter Values (Input Nil)")
	}

	if len(TwitterBotIDToLoad) == 0 {
		return errors.New("Get Requires One or More Filter Values (Input Count 0)")
	}
	...
	for _, v := range TwitterBotIDToLoad {
		if util.LenTrim(filter) > 0 {
			filter += ", "
		}
	...
	if strings.Contains(filter, ",") {
		filter = "WHERE TwitterBotID IN (" + filter + ")"
	} else {
		filter = "WHERE TwitterBotID = " + filter
	}
...
func (l *TwitterBotChildList) GetCustom(filter string, sort string, offsetCount int, limitCount int, args ...interface{}) error {
	// validate
	if l == nil {
		return errors.New("List Object Nil")
	}
...
//		args = optional ordinal parameter arguments to pass into the filter query portion (must appear in ordinal position of params defined within filter)
func (l *TwitterBotChildList) getInternal(filter string, sort string, offsetCount int, limitCount int, args ...interface{}) error {
	// validate
	if l == nil {
		return errors.New("List Object Nil")
	}
	...
	if !isFromOverride {
		q.Set("SELECT * FROM TwitterBotChild " + filter + sort + limit + ";")
	} else {
	...
	output := &[]TwitterBotChild{}

	// execute query
	var dbCurrent *data.MySql

	if !_TwitterBotChildListPrefersDBReader {
		dbCurrent = db
	} else {
		dbCurrent = getReaderDB()
	}
...
func (l *TwitterBotChildList) IsDataChanged() (changedList *[]TwitterBotChild) {
	// validate
	if l == nil {
...
func (l *TwitterBotChildList) Validate() (invalid *TwitterBotChild, invalidInfo string, err error) {
	// validate
	if l == nil {
...
// a new database row will be inserted if such was not found,
// otherwise existing database row will be updated
func (l *TwitterBotChildList) Set() (failed *TwitterBotChild, err error) {
	// validate
	if l == nil {
		// if not valid, then return error
		return nil, errors.New("List Object Nil")
...
func (l *TwitterBotChildList) Delete() (failed *TwitterBotChild, err error) {
	// validate
	if l == nil {
		// if not valid, then return error
		return nil, errors.New("List Object Nil")
...
// [ Notes ]
//		this function is based on the database column dbAttr:"GetBy" for suffix naming
func (l *TwitterBotChildList) FindByID(IDToFind ...int64) (foundList *[]TwitterBotChild, err error) {
	// validate
	if l == nil {
		// if not valid, then return error
		return nil, errors.New("List Object Nil")
...
// FindByTwitterBotID will search existing struct slice for one or more matching values of same parameter,
// it will return a list of found objects, or error if encountered
// [ Parameters ]
//		TwitterBotIDToFind = variadic, one or more table TwitterBotID to find, for example, enter 2, 3, 6, 9, will find records with TwitterBotID 2, 3, 6, and 9 in list
// [ Notes ]
//		this function is based on the database column dbAttr:"GetBy" for suffix naming
func (l *TwitterBotChildList) FindByTwitterBotID(TwitterBotIDToFind ...int64) (foundList *[]TwitterBotChild, err error) {
	// validate
	if l == nil {
		// if not valid, then return error
		return nil, errors.New("List Object Nil")
	...
	// loop thru list to match
	for _, seek := range TwitterBotIDToFind {
		if seek > 0 {
			for _, v := range *l.List {
				if v.TwitterBotID != 0 {
					if v.TwitterBotID == seek {
						// found match
						*foundList = append(*foundList, v)
					}
				}
